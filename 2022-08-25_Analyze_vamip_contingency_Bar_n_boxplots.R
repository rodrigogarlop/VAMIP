...
plot_longitudinal_distro_per_category <- function(vect, map, genome_classif, item_name="Item", comparison_name="Comparison", min_nonzero=2){ # Input items are a frequency vector with NAs (vect), a list containing positions per group (e.g. months, age, etc.; map), names for the actual vector (mutation are expected; item_name) and the category that is explored (comparison_name), and minimum expected subcategories (groups) that should have the item (min_nonzero). genome_classif is now included to hold a classification of the genomes and should correspond exactly to the same positions found in map
	...
	if(sum(list_totals > 0) >= min_nonzero){
		...
		# list_out <- lapply(map,function(x){temp <- vect[x]; temp <- temp[!is.na(temp)]; if(length(temp)==0){temp <- 0}; temp}) # for each item in the map, extract non-NA values from the vector into a list output. DEPRECATED:this old version calculated everything from scratch
		...
		# bp <- barplot(map_vs_cats/max(list_totals), las=2, main=paste(item_name, "by", comparison_name), ylab="Frequency", col=col, border=NA, yaxt='n', ylim=c(-0.1,1.1)) # Use this to plot totals
...
mut_nt2AA[,"New_name"] <- paste(mut_nt2AA[,"Gene"],mut_nt2AA[,"MutAA"],mut_nt2AA[,"MutType"], sep='_')

# IMPORTANT: This should have the same order of the columns in df for the following lines to work correctly
minimetatab <- t(as.data.frame(str_split(colnames(df), "\\|"))) # Extract a mini metadata table from the names (including
rownames(minimetatab) <- NULL # row names are removed
all_mainvar <- find_items(minimetatab[,2],meta, "Variants") # If it does not exist, create a map of the main variants (VOCs and 519).
# for(mut in 1:ncol(df)){
# 	print(mut)
# 	name <- rownames(df)[1]
# 	print(name)
# 	sub <- sub("\\|.*","",name)
# 	print(sub)
# 	# plot_longitudinal_distro_per_category(unlist(df[mut,]), longitudinal_map_months, all_mainvar, mut, "Month_14_fullname",14)
# }
for(mut in rownames(df)){
# 	print(mut)
	sub <- sub("\\|.*","",mut)
	...
	print(sub)
	plot_longitudinal_distro_per_category(unlist(df[mut,]), longitudinal_map_months, all_mainvar, sub, "Variants_per_month",4)
}

# load("checkpoint1.Rdata")
# vect <- unlist(df[2404,])
# map =longitudinal_map_months
# genome_classif <- all_mainvar
# comparison_name="Comparison"
# item_name="Item"
# min_nonzero=2
# mut <- rownames(df)[2024]
