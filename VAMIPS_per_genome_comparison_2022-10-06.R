...
rownames(df) <- sub("\\|.*","",rownames(df)) # Remove tags for delta and omicron as we are expanding the VOCs we are observing
...
# 	print(round(completenessLin,2))
	testVOC <- completenessVOC>=0.80 # This will be used to decide if this may be a coinfection
	if((grepl("B\\.1\\.1\\.519",name)) && testVOC["222"]){testVOC["222"]<-FALSE} # those identified as 519 will always have all 222's mutations, so we can ignore these
	if((grepl("Omicron",name)) && testVOC["222"]){testVOC["222"]<-FALSE} # similarly, those identified as omicron have 8/10 of 222's mutations
	if((grepl("Delta",name)) && testVOC["222"]){testVOC["222"]<-FALSE}
	if(sum(testVOC)>=2){write.table(rbind(out_tableVOC,expVOC,obsVOC,completenessVOC), paste0("Main_mut_Freq-VOC/", strip(name), ".tsv"), sep="\t", quote=FALSE, row.names=T, col.names=NA)}
	if(sum(completenessLin>0.80)>=2){write.table(rbind(out_tableLin,expLin,obsLin,completenessLin), paste0("Main_mut_Freq-Lineage/", strip(name), ".tsv"), sep="\t", quote=FALSE, row.names=T, col.names=NA)}
}
